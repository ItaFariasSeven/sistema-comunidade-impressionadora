...
def login():
    ...
    if form_login.validate_on_submit() and 'button_submit_login' in request.form:
        ...
        if usuario and bcrypt.check_password_hash(usuario.password, form_login.password.data):
            ...
            flash(f'Falha no Login E-mail ou Senha errados', 'alert-danger')
    if form_criarconta.validate_on_submit() and 'button_submit_criarconta' in request.form:
        senha_cript = bcrypt.generate_password_hash(form_criarconta.password.data).decode('utf-8')
...
@login_required
def perfil():
    foto_perfil = url_for('static', filename=f'fotos_perfil/{current_user.photo_perfil}')
...
def criar_post():
    ...
    if form.validate_on_submit():
        post = Post(title=form.title.data, body=form.body.data, autor=current_user)
...
def editar_perfil():
    ...
    if form.validate_on_submit():
        ...
        if form.photo_perfil.data:
            nome_imagem = salvar_imagem(form.photo_perfil.data)
            current_user.photo_perfil = nome_imagem
